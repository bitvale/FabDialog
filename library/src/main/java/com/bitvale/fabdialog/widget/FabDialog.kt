...
class FabDialog : ConstraintLayout {
    ...
    private fun showDimBackground() {
        ...
        val lp = ViewGroup.LayoutParams(settings.maxWidth, settings.maxHeight)
    ...
    private fun setupCoordinates() {
        addOnLayoutChangeListener(object : OnLayoutChangeListener {
            override fun onLayoutChange(v: View?, left: Int, top: Int, right: Int, bottom: Int, oldLeft: Int, oldTop: Int, oldRight: Int, oldBottom: Int) {
                ...
                val screenCenterY = settings.parentHeight / 2.2f
    ...
    /**
     * Set the icon to display on fab.
     *
     * * @param icon The drawable of the icon
     */
    fun setFabIcon(icon: Drawable?) {
        fubIconBitmap = BitmapUtil.getBitmapFromDrawable(icon)
        if (state == COLLAPSED) invalidate()
    }
    ...
    fun setContentView(@LayoutRes layoutResID: Int) {
        ...
        val scrollView = findViewById<ScrollView>(R.id.sv_msg)
        scrollView.visibility = View.GONE
        val customPanel = findViewById<ViewGroup>(R.id.custom_panel)
        customPanel.removeAllViews()
        LayoutInflater.from(context).inflate(layoutResID, customPanel)
        calculateDialogSize()
    }

    /**
     * Set the dialog view from a layout resource. The resource will be
     *
     * inflated and added to dialog as a top-view and replace default view
     *
     * with title, buttons and message. This method has no effect if called
     *
     * after [expandDialog]
     *
     * &nbsp;
     ...
     * @param layoutResID Resource ID to be inflated.
     */
    fun setCustomDialogView(@LayoutRes layoutResID: Int) {
        if (state == EXPANDED) return
        removeAllViews()
    ...
    /**
     * Set the default dialog layout resource as the top view.
     */
    fun setUseDefaultView() = setCustomDialogView(R.layout.include_dialog_layout)
     ...
     */
    fun setDialogBackgroundColor(@ColorInt color: Int) {
     ...
     * @param color the color of the background
     */
    fun setDimBackgroundColor(@ColorInt color: Int) {
